...
data.frequency = zeros(dims(1), dims(2), nlayers, nviews);
data.phase = zeros(dims(1), dims(2), nlayers, nviews);
data.orientation = zeros(dims(1), dims(2), nlayers, nviews);
        ...
        for lloop = 1:nlayers
            for vloop = 1:nviews
                localPatch = cos2d(stimulusSize(1), ...
                    obj.frequency(loop1d, loop2d, lloop, vloop), ...
                    obj.phase(loop1d, loop2d, lloop, vloop), ...
                    obj.orientation(loop1d, loop2d, lloop, vloop), ...
                    stimulusSize(1)./2, stimulusSize(2)./2);
                if nargout >= 2
                    data.frequency (loop1d, loop2d, lloop, vloop) = obj.frequency(loop1d, loop2d, lloop, vloop);
                    data.phase(loop1d, loop2d, lloop, vloop) = obj.phase(loop1d, loop2d, lloop, vloop);
                    data.orientation(loop1d, loop2d, lloop, vloop) = obj.orientation(loop1d, loop2d, lloop, vloop);
                end
