function [ handle ] = plotBarsWithErrors(varargin)
    ...
    if isfield(inputs, 'names') || isfield(inputs, 'colours')
        handle = bar(gh, inputs.X, inputs.Y, inputs.width, 'LineStyle', 'none'); hold on;        
        for loop = 1:length(inputs.X)
            handle(1,loop) = bar(gh, inputs.X(loop), inputs.Y(loop), inputs.width); hold on;
        ...
        handle(1) = bar(gh, inputs.X, inputs.Y, 1, 'LineWidth', 0.5); hold on;
        %errorbar(1:length(inputs.Y), inputs.Y, inputs.M(1,:), inputs.M(2,:) ,'k-','LineWidth',3);
        handle(2) = errorbar(gh, inputs.X, inputs.Y, inputs.M(1,:), inputs.M(2,:) ,'k.');
